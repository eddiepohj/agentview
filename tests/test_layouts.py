...
def test_tieredrunner_slug_is_the_parent_of_the_state_dir(tmp_path):
    # The direction already got wrong once: tiered-runner names the run one level
    # ABOVE the state dir, plan-runner names it AT the state dir.
    _ledger(tmp_path / "proj/_tieredrunner/demo-run/state/ledger.json")
    _ledger(tmp_path / "proj/_tieredrunner/soak-2/state/ledger.json")
...
def test_tieredrunner_takes_the_unsuffixed_build_log(tmp_path):
    _ledger(tmp_path / "proj/_tieredrunner/demo-run/state/ledger.json")
...
def test_tieredrunner_gate_rounds_are_nested_per_step(tmp_path):
    # gate.py write_round drops <emit_dir>/<step>/<gate>-r<n>.json, and the
    # skill passes --emit-dir _tieredrunner/<slug>/state/md --step <id>.
    _ledger(tmp_path / "proj/_tieredrunner/demo-run/state/ledger.json")
    (tmp_path / "proj/_tieredrunner/demo-run/state/md/S4").mkdir(parents=True)
...
def test_legacy_maxrunner_runs_remain_readable(tmp_path):
    _ledger(tmp_path / "proj/_maxrunner/legacy-run/state/ledger.json")
    runs = find_runs(tmp_path)
    assert len(runs) == 1
    assert runs[0].slug == "legacy-run"
    assert runs[0].layout.marker == "_maxrunner"
...
def test_only_a_runner_with_an_md_tier_declares_a_gate_thread():
    ...
    assert BY_MARKER["_tieredrunner"].gate_thread.format(slug="demo-run") == \
        "cmag-demo-run"


def test_declared_models_match_the_skill_taxonomy():
    tiers = {t.role: t for t in BY_MARKER["_tieredrunner"].tiers}
